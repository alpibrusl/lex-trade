...
fn sample_order() -> order.Order {
  order.order("ORD-2026-001", "MSFT", OrderBuy(()), 100, LimitOrder("125.50"), "0", "ACCOUNT-A", "TRADER-01", "20260527-09:30:00.000")
}
...
fn run_example() -> Str {
  ...
  match result {
    ...
    Rejected(reasons) => str.concat("order rejected: ", list.fold(reasons, "", fn (acc :: Str, r :: rejection.RejectionReason) -> Str {
      if acc == "" {
        rejection.describe(r)
      } else {
        str.concat(acc, str.concat(", ", rejection.describe(r)))
      }
    })),
  }
}
