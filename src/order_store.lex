...
fn fetch(db :: conn.ConnDb, cl_ord_id :: Str, account :: Str) -> [sql] Result[Option[lc.OrderState], dbe.DbErr] {
  let sq := q.for_dialect({ sql: "SELECT state_kind, filled_qty, avg_price_str, reason FROM order_states WHERE cl_ord_id = ? AND account = ?", params: [PStr(cl_ord_id), PStr(account)] }, db.dialect)
  let raw :: Result[List[{ state_kind :: Str, filled_qty :: Int, avg_price_str :: Str, reason :: Str }], SqlError] := sql.query(db.handle, sq.sql, sq.params)
  match raw {
    Err(e) => Err(dbe.sql_error(match e.code {
      None => "",
      Some(c) => c,
...
fn upsert(db :: conn.ConnDb, cl_ord_id :: Str, account :: Str, symbol :: Str, state :: lc.OrderState) -> [sql] Result[Unit, dbe.DbErr] {
  let encoded := encode_state(state)
  let sq := q.for_dialect({ sql: "INSERT INTO order_states (cl_ord_id, account, symbol, state_kind, filled_qty, avg_price_str, reason) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT (cl_ord_id, account) DO UPDATE SET state_kind = EXCLUDED.state_kind, filled_qty = EXCLUDED.filled_qty, avg_price_str = EXCLUDED.avg_price_str, reason = EXCLUDED.reason", params: [PStr(cl_ord_id), PStr(account), PStr(symbol), PStr(match encoded {
    (k, _, _, _) => k,
  }), PInt(match encoded {
    (_, fq, _, _) => fq,
  }), PStr(match encoded {
...
fn apply_event(db :: conn.ConnDb, cl_ord_id :: Str, account :: Str, symbol :: Str, event :: lc.OrderEvent) -> [sql] Result[lc.OrderState, dbe.DbErr] {
          ...
          if row.state_kind == "PendingCancel" {
            ...
            if row.state_kind == "Canceled" {
              ...
              if row.state_kind == "Rejected" {
                Ok(ExchangeRejected(row.reason))
              } else {
