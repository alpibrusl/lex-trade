...
fn validate_cancel(db :: conn.ConnDb, cl_ord_id :: Str, orig_cl_ord_id :: Str, account :: Str, symbol :: Str, side :: order.OrderSide, order_qty :: Int, transact_time :: Str, sender :: Str, target :: Str) -> [sql] Result[ocr.OrderCancelRequest, rejection.RejectionReason] {
  match ostore.fetch(db, orig_cl_ord_id, account) {
    ...
    Ok(opt_state) => {
      ...
      if not lc.is_cancelable(state) {
        ...
        Ok({ cl_ord_id: cl_ord_id, orig_cl_ord_id: orig_cl_ord_id, symbol: symbol, side: fix_side, order_qty: order_qty, transact_time: transact_time, sender_comp_id: sender, target_comp_id: target, account: Some(account), text: None })
      }
    },
  }
}
