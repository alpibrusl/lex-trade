...
type ValidationResult =
    Accepted(nos.NewOrderSingle)
  | Rejected(List[rejection.RejectionReason])
...
fn validate(
  ...
  # Step 1 — collect risk-limit violations
  let v0 := []
  ...
  # Step 2 — build the typed NewOrderSingle
  let nos_val := order.order_to_nos(o, sender, target)

  # Step 3 — FIX conformance check (operates on FixMessage)
  let fix_msg := nos.to_fix_message(nos_val, 1)
  let v4 := match conf.validate_new_order(fix_msg) {
    Ok(_)    => v3,
    Err(errs) => list.concat(v3,
                   [FixConformanceFailure(conf.describe_errors(errs))]),
  }
