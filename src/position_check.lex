...
type PositionCheckConfig = { max_notional :: d.Decimal, allow_flip :: Bool }
  ...
  match pstore.fetch(db, key) {
    ...
    Ok(current) => {
      ...
      let signed_qty := match o.side {
        OrderBuy(_) => o.quantity,
        OrderSell(_) => 0 - o.quantity,
      }
      let projected_qty := current.qty + signed_qty
      let projected_pos := { key: current.key, qty: projected_qty, avg_cost: current.avg_cost, realized_pnl: current.realized_pnl }
      let notional := exp.gross_notional(projected_pos, mark)
      if not d.is_zero(mark) and d.gt(notional, config.max_notional) {
        let msg := str.concat("projected notional ", str.concat(pos.decimal_to_str(notional), str.concat(" exceeds limit ", pos.decimal_to_str(config.max_notional))))
