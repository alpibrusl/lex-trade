...
fn sample_order() -> order.Order {
  order.order("ORD-001", "MSFT", OrderBuy(()), 100, LimitOrder("125.50"), "0", "ACCOUNT-A", "TRADER-01", "20260528-09:30:00.000")
}
...
fn run_validation(o :: order.Order, lim :: limit.RiskLimit) -> [sql, fs_write, time] Result[{ result :: v.ValidationResult, event_count :: Int }, Str] {
  match trail_log.open_memory() {
    Err(e) => Err(e),
    Ok(log) => {
      let result := vio.validate_and_log(o, lim, None, { max_deviation_bps: 200 }, "ALGO01", "EXCH01", log)
...
fn test_rejected_result() -> [sql, fs_write, time] Result[Unit, Str] {
  let o := order.order("ORD-002", "MSFT", OrderBuy(()), 99999, LimitOrder("125.50"), "0", "ACCOUNT-A", "TRADER-01", "20260528-09:30:00.000")
...
fn test_rejected_logs_two_events() -> [sql, fs_write, time] Result[Unit, Str] {
  let o := order.order("ORD-003", "MSFT", OrderBuy(()), 99999, LimitOrder("125.50"), "0", "ACCOUNT-A", "TRADER-01", "20260528-09:30:00.000")
  match run_validation(o, default_limits()) {
    Err(e) => fail(e),
...
fn test_two_orders_log_four_events() -> [sql, fs_write, time] Result[Unit, Str] {
  let o2 := order.order("ORD-999", "AAPL", OrderSell(()), 50, MarketOrder(()), "0", "ACCOUNT-B", "TRADER-02", "20260528-09:31:00.000")
  match trail_log.open_memory() {
    Err(e) => fail(e),
    Ok(log) => {
      let __lex_discard_1 := vio.validate_and_log(sample_order(), default_limits(), None, { max_deviation_bps: 200 }, "ALGO01", "EXCH01", log)
      let __lex_discard_2 := vio.validate_and_log(o2, default_limits(), None, { max_deviation_bps: 200 }, "ALGO02", "EXCH01", log)
      let count := match trail_log.range(log, 0, 9999999999999) {
        Err(_) => 0,
        Ok(evts) => list.len(evts),
      }
...
fn suite() -> [sql, fs_write, time] List[Result[Unit, Str]] {
  [test_accepted_result(), test_accepted_logs_two_events(), test_rejected_result(), test_rejected_logs_two_events(), test_two_orders_log_four_events()]
}
