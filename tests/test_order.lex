...
fn test_order_to_ord_type_market() -> Result[Unit, Str] {
  ...
  match t {
    Market    => pass(),
    Limit     => fail("expected Market"),
    Stop      => fail("expected Market"),
    StopLimit => fail("expected Market"),
  }
}
...
fn test_order_to_ord_type_limit() -> Result[Unit, Str] {
  ...
  match t {
    Limit     => pass(),
    Market    => fail("expected Limit"),
    Stop      => fail("expected Limit"),
    StopLimit => fail("expected Limit"),
  }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    test_order_to_fix_side_buy(),
    test_order_to_fix_side_sell(),
    test_order_to_ord_type_market(),
    test_order_to_ord_type_limit(),
    test_order_price_limit(),
    test_order_price_market(),
    test_order_to_nos(),
  ]
}
