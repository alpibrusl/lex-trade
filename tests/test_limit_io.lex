...
fn test_store_and_fetch_qty() -> [sql, fs_write] Result[Unit, Str] {
  match open_db() {
    Err(e) => fail(e),
    Ok(db) => {
      let lim := { max_order_qty: 500, max_notional_str: "250000.00", allowed_symbols: ["MSFT", "AAPL"], allowed_sides: ["buy"] }
...
fn test_store_and_fetch_notional() -> [sql, fs_write] Result[Unit, Str] {
  match open_db() {
    Err(e) => fail(e),
    Ok(db) => {
      let lim := { max_order_qty: 500, max_notional_str: "250000.00", allowed_symbols: [], allowed_sides: ["buy", "sell"] }
...
fn test_allowed_symbols_roundtrip() -> [sql, fs_write] Result[Unit, Str] {
  match open_db() {
    Err(e) => fail(e),
    Ok(db) => {
      let lim := { max_order_qty: 1000, max_notional_str: "1000000.00", allowed_symbols: ["MSFT", "GOOG", "AAPL"], allowed_sides: ["buy", "sell"] }
...
fn test_empty_symbols_means_all_allowed() -> [sql, fs_write] Result[Unit, Str] {
  match open_db() {
    Err(e) => fail(e),
    Ok(db) => {
      let lim := { max_order_qty: 1000, max_notional_str: "1000000.00", allowed_symbols: [], allowed_sides: ["buy", "sell"] }
...
fn suite() -> [sql, fs_write] List[Result[Unit, Str]] {
  [test_fetch_returns_default_when_missing(), test_fetch_default_has_buy_and_sell(), test_store_and_fetch_qty(), test_store_and_fetch_notional(), test_allowed_symbols_roundtrip(), test_empty_symbols_means_all_allowed(), test_store_overwrites(), test_remove_reverts_to_defaults(), test_two_accounts_independent()]
}
