...
fn base_order() -> order.Order {
  order.order("CL-001", "AAPL", OrderBuy(()), 100, LimitOrder("150.00"), "0", "ACC1", "TRADER-01", "20260528-09:30:00.000")
}
...
fn test_valid_replace_passes() -> Result[Unit, Str] {
  let orig := base_order()
  let amended := order.order("CL-002", "AAPL", OrderBuy(()), 90, LimitOrder("151.00"), "0", "ACC1", "TRADER-01", "20260528-09:31:00.000")
...
fn test_replace_changing_side_is_rejected() -> Result[Unit, Str] {
  let orig := base_order()
  let amended := order.order("CL-002", "AAPL", OrderSell(()), 100, LimitOrder("150.00"), "0", "ACC1", "TRADER-01", "20260528-09:31:00.000")
  match replace.validate_replace(orig, amended, lim(), "ALGO01", "EXCH01") {
...
fn test_replace_changing_symbol_is_rejected() -> Result[Unit, Str] {
  let orig := base_order()
  let amended := order.order("CL-002", "MSFT", OrderBuy(()), 100, LimitOrder("300.00"), "0", "ACC1", "TRADER-01", "20260528-09:31:00.000")
  match replace.validate_replace(orig, amended, lim(), "ALGO01", "EXCH01") {
...
fn test_replace_exceeding_qty_limit_is_rejected() -> Result[Unit, Str] {
  let orig := base_order()
  let amended := order.order("CL-002", "AAPL", OrderBuy(()), 99999, LimitOrder("150.00"), "0", "ACC1", "TRADER-01", "20260528-09:31:00.000")
  match replace.validate_replace(orig, amended, lim(), "ALGO01", "EXCH01") {
...
fn test_replace_changing_account_is_rejected() -> Result[Unit, Str] {
  let orig := base_order()
  let amended := order.order("CL-002", "AAPL", OrderBuy(()), 100, LimitOrder("150.00"), "0", "ACC2", "TRADER-01", "20260528-09:31:00.000")
  match replace.validate_replace(orig, amended, lim(), "ALGO01", "EXCH01") {
...
fn suite() -> List[Result[Unit, Str]] {
  [test_valid_replace_passes(), test_replace_changing_side_is_rejected(), test_replace_changing_symbol_is_rejected(), test_replace_exceeding_qty_limit_is_rejected(), test_replace_changing_account_is_rejected()]
}
