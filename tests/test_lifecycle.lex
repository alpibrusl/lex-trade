...
fn test_pending_new_reject() -> Result[Unit, Str] {
  match lc.transition(PendingNew(()), ExchangeReject("no market")) {
    Ok(ExchangeRejected(_)) => pass(),
    Ok(_) => fail("expected ExchangeRejected"),
    Err(e) => fail(e),
  }
}
...
fn test_rejected_rejects_events() -> Result[Unit, Str] {
  let state := ExchangeRejected("bad order")
...
fn test_is_terminal() -> Result[Unit, Str] {
  ...
  let t3 := lc.is_terminal(ExchangeRejected("x"))
...
fn sample_er(exec_type :: en.ExecType) -> er.ExecutionReport {
  { exec_id: "E001", order_id: "ORD-001", cl_ord_id: "CL-001", exec_type: exec_type, ord_status: StatusNew(()), symbol: "AAPL", side: Buy(()), order_qty: "100", cum_qty: "0", leaves_qty: "100", avg_px: "0", last_px: "", last_qty: "", text: "" }
}
...
fn test_from_er_exec_fill() -> Result[Unit, Str] {
  let report := { exec_id: "E002", order_id: "ORD-001", cl_ord_id: "CL-001", exec_type: ExecFill(()), ord_status: StatusFilled(()), symbol: "AAPL", side: Buy(()), order_qty: "100", cum_qty: "100", leaves_qty: "0", avg_px: "10.50", last_px: "10.50", last_qty: "100", text: "" }
...
fn suite() -> List[Result[Unit, Str]] {
  [test_pending_new_ack(), test_pending_new_reject(), test_pending_new_fill_is_invalid(), test_new_partial_fill(), test_new_full_fill(), test_new_cancel_ack(), test_new_duplicate_ack_is_error(), test_partially_filled_additional_fill(), test_partially_filled_to_filled(), test_partially_filled_cancel(), test_filled_rejects_events(), test_canceled_rejects_events(), test_rejected_rejects_events(), test_is_cancelable(), test_is_terminal(), test_from_er_exec_new(), test_from_er_exec_fill(), test_from_er_exec_canceled(), test_from_er_pending_new_is_err()]
}
