...
fn sample_order() -> order.Order {
  order.order("ORD-001", "MSFT", OrderBuy(()), 100, LimitOrder("125.50"), "0", "ACCOUNT-A", "TRADER-01", "20260527-09:30:00.000")
}
...
fn test_exceeds_qty_rejected() -> Result[Unit, Str] {
  let o := order.order("ORD-002", "MSFT", OrderBuy(()), 99999, LimitOrder("125.50"), "0", "ACCOUNT-A", "TRADER-01", "20260527-09:30:00.000")
  let limits := limit.default_limits()
  let result := v.validate(o, limits, "ALGO01", "EXCH01")
...
fn test_exceeds_qty_has_correct_reason() -> Result[Unit, Str] {
  let o := order.order("ORD-003", "MSFT", OrderBuy(()), 99999, LimitOrder("125.50"), "0", "ACCOUNT-A", "TRADER-01", "20260527-09:30:00.000")
  let limits := limit.default_limits()
  let result := v.validate(o, limits, "ALGO01", "EXCH01")
  match result {
    Accepted(_) => fail("should have been rejected"),
    Rejected(reasons) => {
      let has_qty := list.fold(reasons, false, fn (acc :: Bool, r :: rejection.RejectionReason) -> Bool {
        if acc {
          true
        } else {
          match r {
...
fn test_symbol_blocked_rejected() -> Result[Unit, Str] {
  let o := sample_order()
  let limits := { max_order_qty: 10000, max_notional_str: "5000000.00", allowed_symbols: ["AAPL", "GOOG"], allowed_sides: ["buy", "sell"] }
  let result := v.validate(o, limits, "ALGO01", "EXCH01")
...
fn test_is_rejected_true() -> Result[Unit, Str] {
  let o := order.order("ORD-004", "MSFT", OrderBuy(()), 99999, LimitOrder("125.50"), "0", "ACCOUNT-A", "TRADER-01", "20260527-09:30:00.000")
  let limits := limit.default_limits()
  let result := v.validate(o, limits, "ALGO01", "EXCH01")
...
fn suite() -> List[Result[Unit, Str]] {
  [test_valid_order_accepted(), test_exceeds_qty_rejected(), test_exceeds_qty_has_correct_reason(), test_symbol_blocked_rejected(), test_is_accepted_true(), test_is_rejected_true(), test_accepted_order_some()]
}
