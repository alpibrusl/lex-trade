...
fn test_cancel_new_order_succeeds() -> [orders, sql, fs_write] Result[Unit, Str] {
  match open_db() {
    Err(msg) => fail(msg),
    Ok(db)   => {
      let _ := ostore.upsert(db, "CL-001", "ACC1", "AAPL", New)
...
fn test_cancel_partially_filled_succeeds() -> [orders, sql, fs_write] Result[Unit, Str] {
  match open_db() {
    Err(msg) => fail(msg),
    Ok(db)   => {
      let _ := ostore.upsert(db, "CL-002", "ACC1", "AAPL", PartiallyFilled(50, price(1000, -2)))
...
fn test_cancel_filled_order_is_rejected() -> [orders, sql, fs_write] Result[Unit, Str] {
  match open_db() {
    Err(msg) => fail(msg),
    Ok(db)   => {
      let _ := ostore.upsert(db, "CL-003", "ACC1", "AAPL", Filled(price(1050, -2)))
...
fn test_cancel_canceled_order_is_rejected() -> [orders, sql, fs_write] Result[Unit, Str] {
  match open_db() {
    Err(msg) => fail(msg),
    Ok(db)   => {
      let _ := ostore.upsert(db, "CL-004", "ACC1", "AAPL", Canceled(50))
...
fn suite() -> [orders, sql, fs_write] List[Result[Unit, Str]] {
  [
    test_cancel_unknown_order_is_rejected(),
    test_cancel_new_order_succeeds(),
    test_cancel_partially_filled_succeeds(),
    test_cancel_filled_order_is_rejected(),
    test_cancel_canceled_order_is_rejected(),
    test_order_store_apply_event_pipeline(),
  ]
}
