...
fn test_market_order_always_passes() -> Result[Unit, Str] {
  let o   := market_order()
  let ref := price(17500, -2)   # 175.00
  match pc.check_price_tolerance(o, ref, tolerance_200bps()) {
    PriceOk           => pass(),
    PriceRejected(_)  => fail("market order should always pass"),
  }
}

fn test_limit_within_tolerance_passes() -> Result[Unit, Str] {
  # ref = 175.00, order = 174.00, deviation = 100/17500 * 10000 = 57bps < 200bps
  let o   := limit_order("174.00")
  let ref := price(17500, -2)
  match pc.check_price_tolerance(o, ref, tolerance_200bps()) {
    PriceOk          => pass(),
    PriceRejected(_) => fail("174.00 vs 175.00 is within 200bps"),
  }
}

fn test_limit_at_boundary_passes() -> Result[Unit, Str] {
  # ref = 100.00, tolerance = 200bps = 2%
  # 200bps deviation: order = 102.00 (exactly 200bps above)
  # lhs = |10200-10000| * 10000 = 200 * 10000 = 2000000
  # rhs = 10000 * 200 = 2000000
  # lhs > rhs is false, so PriceOk
  let o   := limit_order("102.00")
  let ref := price(10000, -2)
  match pc.check_price_tolerance(o, ref, tolerance_200bps()) {
    PriceOk          => pass(),
    PriceRejected(_) => fail("102.00 vs 100.00 is exactly 200bps, should pass"),
  }
}

fn test_limit_exceeds_tolerance_is_rejected() -> Result[Unit, Str] {
  # ref = 100.00, order = 103.00 = 300bps above → rejected
  let o   := limit_order("103.00")
  let ref := price(10000, -2)
  match pc.check_price_tolerance(o, ref, tolerance_200bps()) {
    PriceOk           => fail("103.00 vs 100.00 is 300bps, should be rejected"),
    PriceRejected(r)  => match r {
...
fn test_limit_below_tolerance_is_rejected() -> Result[Unit, Str] {
  # ref = 100.00, order = 97.00 = 300bps below → rejected
  let o   := limit_order("97.00")
  let ref := price(10000, -2)
  match pc.check_price_tolerance(o, ref, tolerance_200bps()) {
    PriceOk           => fail("97.00 vs 100.00 is 300bps, should be rejected"),
    PriceRejected(r)  => match r {
...
fn test_exact_ref_price_passes() -> Result[Unit, Str] {
  ...
  match pc.check_price_tolerance(o, ref, tolerance_200bps()) {
    PriceOk          => pass(),
    PriceRejected(_) => fail("exact match should pass"),
  }
}
...
fn test_zero_tolerance_rejects_any_deviation() -> Result[Unit, Str] {
  ...
  match pc.check_price_tolerance(o, ref, zero_tol) {
    PriceOk          => fail("any deviation should fail with 0bps tolerance"),
    PriceRejected(_) => pass(),
  }
}

fn test_stop_limit_order_checks_limit_price() -> Result[Unit, Str] {
  # StopLimitOrder(stop_price, limit_price) — we check the limit price
  let o := order.order(
    "CL-002", "AAPL", OrderBuy, 100,
    StopLimitOrder("175.00", "103.00"), "0",
    "ACC1", "TRADER-01", "20260528-09:30:00.000"
  )
  let ref := price(10000, -2)
  match pc.check_price_tolerance(o, ref, tolerance_200bps()) {
    PriceOk          => fail("stop limit: limit price 103.00 is 300bps over ref"),
    PriceRejected(_) => pass(),
  }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [
    test_market_order_always_passes(),
    test_limit_within_tolerance_passes(),
    test_limit_at_boundary_passes(),
    test_limit_exceeds_tolerance_is_rejected(),
    test_limit_below_tolerance_is_rejected(),
    test_exact_ref_price_passes(),
    test_zero_tolerance_rejects_any_deviation(),
    test_stop_limit_order_checks_limit_price(),
  ]
}
